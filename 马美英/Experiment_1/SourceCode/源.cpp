...

{
	...
	for (int i = 0; i <N; i++)
	{
		if (a==stu[i].number)
		{
			cout << "符合学号;" << stu[i].number << "的是 " << "姓名:" << stu[i].name << " " << "平时成绩：" << stu[i].dailyScore << " " << "实验成绩:" << stu[i].experimentScore << " " << "期末成绩:" << stu[i].finalScore << " " << "总成绩:" << stu[i].generalScore << " " << "排名" << stu[i].place << endl;
			b = 1;
